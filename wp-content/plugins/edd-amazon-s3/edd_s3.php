...
<?php
	...
	public function get_s3_url( $filename, $expires = 5, $bucket = 'caldera-learn-videos' ) {
		...
		if( false !== strpos( $filename, '/' ) ) {
			...
			if ( empty( $buckets ) ) {
				$errors = edd_get_errors();
				if ( is_array( $errors ) && array_key_exists( 'edd-amazon-s3', $errors ) ) {
					if ( current_user_can( 'manage_options' ) ) {
		...
		if( false !== strpos( $filename, $bucket ) ) {
			$filename = str_replace( $bucket . '/', '', $filename );
		}
